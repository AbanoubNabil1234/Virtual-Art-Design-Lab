...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-gray-50/50">
      ...
      <main class="max-w-4xl mx-auto p-4 md:p-8 animate-fade-in" *ngIf="topic(); else missingTopic">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <nav class="flex items-center gap-2 text-sm font-bold">
            ...
            <span class="text-gray-900 truncate max-w-[240px]">{{ topic()?.lessonNumber }}</span>
          </nav>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-10 transition-shadow hover:shadow-md">
          <div class="bg-gradient-to-r from-secondary/5 to-transparent p-6 md:p-8 border-b border-gray-100">
            <div class="flex items-center gap-3 mb-4">
              <span class="bg-primary/10 text-primary text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-widest">
                {{ topic()?.lessonNumber }}
              </span>
          ...
          <div class="p-6 md:p-8">
            <div class="prose prose-lg max-w-none text-gray-700 leading-relaxed font-medium">
              ...
              <div class="mt-6 bg-gray-50 border border-gray-100 rounded-2xl p-5">
                <h2 class="text-base font-black text-gray-900 mb-3">محاور مقترحة للنقاش</h2>
                <ul class="list-disc pr-6 space-y-2 text-gray-600">
                  <li *ngFor="let prompt of topic()?.prompts">{{ prompt }}</li>
                </ul>
              </div>
            </div>
            ...
            <div class="mt-10 flex items-center gap-6 pt-6 border-t border-gray-50">
              <div class="flex items-center gap-2 text-gray-400">
                ...
                <span class="text-sm font-bold">{{ topic()?.views }} مشاهدة</span>
              </div>
              <div class="flex items-center gap-2 text-gray-400">
                <span class="material-icons text-xl">comment</span>
                <span class="text-sm font-bold">{{ topic()?.replies?.length }} ردود</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6 mb-12">
          <div class="flex items-center gap-3 mb-6">
            ...
            <span class="bg-gray-100 text-gray-500 text-xs font-bold px-3 py-1 rounded-full">
              {{ topic()?.replies?.length }} نقاش
            </span>
          </div>

          <div *ngIf="topic()?.replies?.length === 0" class="py-14 text-center bg-white rounded-3xl border-2 border-dashed border-gray-200">
            <span class="material-icons text-5xl text-gray-200 mb-3">chat_bubble_outline</span>
            <p class="text-gray-500 font-bold">لا توجد أي مشاركة بعد</p>
          </div>

          <div
            *ngFor="let reply of topic()?.replies; let i = index"
          ...
          <div class="p-8 space-y-6">
              ...
              <div class="space-y-2">
                ...
                <div class="relative">
                  ...
                  <input
                    ...
                    class="w-full pr-12 pl-4 py-3 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-primary/20 transition-all font-bold text-gray-700">
                </div>
              </div>
            ...
            <div class="space-y-2">
              ...
              <textarea
                ...
                placeholder="اكتب ردك أو استفسارك المرتبط بهذا الدرس..."
            ...
            <div class="flex justify-end pt-4 border-t border-gray-50">
              <button (click)="addReply()" class="btn-action px-10 shadow-lg py-4" data-sound="submit">
      ...
      <ng-template #missingTopic>
        <main class="max-w-4xl mx-auto p-8 text-center">
          <div class="py-16 bg-white rounded-3xl border border-gray-100">
            <span class="material-icons text-6xl text-gray-200 mb-4">forum</span>
            <h1 class="text-2xl font-black text-gray-900 mb-4">هذا النقاش غير موجود</h1>
            <a routerLink="/forum" class="text-primary font-bold">العودة للمنتدى</a>
          </div>
        </main>
      </ng-template>
    </div>
